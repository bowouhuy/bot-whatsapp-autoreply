...
const jokeList = [
`
Kenapa zombi kalau nyerangnya rame-rame?
Karena kalu sendiri namanya zomblo. xixixix`,

`Sahabat dekat biasanya akan mengajak kita makan kepiting bareng. karena sahabat yang dekat adalah sahabat a crab`,

`Artis siapa yang sering emosian? 
Anjasmaramara.`,

`Cintaku padamu seperti utang. Awalnya kecil, didiemin, tau-tau gede sendiri.`,

`Barusan saya ke apotik beli obat tidur. pulangnya saya bawa pelan peklan takut obatnya bangun`
...
// A kiss is so dear, a car is too dear and a monkey is U dear.`
	...
	const start = (_chats, cnt = 0) => {
		...
		if (lastMsg.toUpperCase().indexOf('HAI') > -1){
			sendText = `
				Hai ${title}! Coba ketik salah satu :
		- TIME
		- JOKES`
		}
		...
		if (lastMsg.toUpperCase().indexOf('JOKES') > -1){
